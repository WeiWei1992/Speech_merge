...
from mp3_to_wav import delete_mp3
...
def _ui():
    ...
    title = Label(root, text="   mp3删除工具", compound=CENTER, font=("微软雅黑", 20))
    ...
    def click():
        ...
        th = threading.Thread(target=delete_mp3, args=(path_result,))
        th.setDaemon(True)
        th.start()
        # baidu_speech_merge(text,path_excel,path_result)

    click_btn = Button(root, text="开始删除", command=click)
